...
- **NetFirewall.Services** — business logic + data access (Npgsql + RepoDb ORM, `UsePostgreSql()` global config). Subsystems:
  ...
  - `Diagnostics/`: daemon-executed, read-only tools behind `IDiagnosticRunner` (gate slot → `diag_runs` row → time budget): PingProbeService (fwmark/iface), RouteOracleService (`ip -j route get|rule|route`), ConntrackLookupService, DropLogService (`journalctl -k`; `dmesg` is blocked by `ProtectKernelLogs`), InterfaceHealthService, SysctlSanityService, WgLiveReader, and `Doctors/` — four checklists on one engine (`IDoctorCheck<TContext>` + `DoctorRunner`: bounded fan-out, per-check deadline, overrun ⇒ Skip): `Vpn/` (VPN doctor + data-plane probe + issued-config compare), `Doctors/Wan` (uplinks, failover config/verdict, per-WAN marks, default-route owner, ping by mark), `Doctors/Dhcp` (unit, UDP/67, scopes vs their interface, pool pressure, lease activity), `Doctors/Dns` (resolver unit, listeners, raw-UDP query locally and per LAN address, upstreams). Invasive tools (Admin + TOTP, one at a time via `IDiagnosticJobRegistry`): `Trace/` **flow inspector** — owns the table `ip netfw_diag` at the raw hook and deletes it in a `finally` plus a startup sweep, never edits the operator's chains — and `Capture/` bounded tcpdump to `/var/lib/netfirewall/daemon/captures` (needs `AF_PACKET` in the daemon unit). Tools MUST spawn via the ArgumentList overload of `IProcessRunner` (enforced by `NoInterpolatedProcessArgsTest`) and validate through `IDiagnosticInputValidator` on both sides. See `docs/diagnostics.md`.
