...
-- All auth tables use IF NOT EXISTS so re-applying the schema does NOT wipe
-- credentials, sessions, or audit history. Add migrations explicitly when the
-- shape changes.
-- ============================================================================
...
-- secret_encrypted layout: 12-byte nonce || 16-byte tag || ciphertext (AES-256-GCM).
CREATE TABLE IF NOT EXISTS user_totp_secrets (
    user_id          uuid                     PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
    secret_encrypted bytea                    NOT NULL,
    enrolled_at      timestamp with time zone NOT NULL DEFAULT now(),
    last_used_at     timestamp with time zone,
    last_used_step   bigint                              -- TOTP step counter; rejects same-window replays
);

-- Recovery codes — single-use, hashed (argon2). Hand the user 10 at enrollment.
...
-- SHA-256 hash so DB-read access does not equal session takeover.
CREATE TABLE IF NOT EXISTS user_sessions (
    id              uuid                     DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id         uuid                     NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    token_hash      text                     NOT NULL UNIQUE,            -- sha256(cookie value)
    auth_level      text                     NOT NULL DEFAULT 'basic',   -- 'basic' | 'elevated'
...
CREATE TABLE IF NOT EXISTS auth_audit_log (
    ...
    username     text,                                                    -- captured for forensics if user is later deleted
    event_type   text                     NOT NULL,                       -- e.g. login.success, login.failed, totp.verified, elevation.granted
    ip           inet,
    user_agent   text,
    detail       jsonb
);
